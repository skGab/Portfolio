<p align="center">
  <a href="" rel="noopener">
    <img width=200px height=200px src="./src/lib/imgs/app_doc.png" alt="Logo do Projeto">
  </a>
</p>

<h3 align="center">Portfólio</h3>

<div align="center">

[![Status](https://img.shields.io/badge/status-active-success.svg)]()
[![GitHub Issues](https://img.shields.io/github/issues/zDeep10/Portfolio.svg)](https://github.com/zDeep10/Portfolio/issues)
[![GitHub Pull Requests](https://img.shields.io/github/issues-pr/zDeep10/Portfolio.svg)](https://github.com/zDeep10/Portfolio/pulls)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](/LICENSE)

</div>

---

<p align="center"> Algumas linhas descrevendo o seu projeto.
  <br> 
</p>

## 📝 Índice

1. [Introdução & O que o sistema deve fazer (objetivos)](#introduction)
2. [Contexto & Escopo (escopo)](#scope)
3. [Hospedagem (implantação)](#deployment)
4. [Requerimentos de qualidade (requisitos de qualidade)](#quality-requirements)
5. [Riscos e falhas conhecidas (riscos e débito técnico)](#risks-and-technical-debt)
6. [Glossário (glossário)](#glossary)
7. [Autor](#authors)

## 1. 🧐 Introdução & O que o sistema deve fazer (objetivos) <a name="introduction"></a>

Escreva 1-2 parágrafos descrevendo o propósito e os objetivos do seu projeto.

## 2. 🌐 Contexto & Escopo (escopo) <a name="scope"></a>

Defina o contexto e o escopo do seu projeto. O que está incluído e o que não está?

## 3. 🚀 Hospedagem (implantação) <a name="deployment"></a>

Explique o processo de implantação e quaisquer observações adicionais para hospedar o sistema.

## 4. 🔍 Requerimentos de qualidade (requisitos de qualidade) <a name="quality-requirements"></a>

Liste os requisitos e critérios de qualidade que o sistema deve atender.

## 5. ⚠️ Riscos e falhas conhecidas (riscos e débito técnico) <a name="risks-and-technical-debt"></a>

Identifique quaisquer riscos conhecidos ou débito técnico que possam impactar o projeto.

## 6. 📖 Glossário (glossário) <a name="glossary"></a>

Forneça definições e explicações para os termos-chave usados no projeto.

## 7. Autor <a name="authors"></a>

-   [@Gabriel Assunção](https://github.com/zDeep10) - Ideia e Construção
